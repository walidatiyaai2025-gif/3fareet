...
| Race/AI | 🟡 In review | `URAC-002→005` stacked implementation now covers ordered checkpoints, one-lap finish, deterministic ranking, countdown/results/restart; Unity execution still pending CI licensing |
...
- `URAC-005` — branch `agent/URAC-005-race-round-flow`: deterministic Countdown → Racing → Results orchestration plus Restart that resets lap/checkpoint state for a new round.
...
4. `URAC-006`: track bounds/barriers/off-road, then `URAC-008/010` AI race robustness.
...
| STS-U04 | 🟡 Medium | `URAC-002→005` code IN REVIEW; integration still needs exact-head Unity execution and later scene wiring | Race Engineer / QA |
