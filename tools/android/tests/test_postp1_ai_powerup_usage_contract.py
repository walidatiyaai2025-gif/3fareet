...
class PostP1AiPowerUpUsageContractTests(unittest.TestCase):
    ...
    def test_public_inventory_path_keeps_fail_closed_validation_and_shares_core(self):
        source = self._read("unity_game/Assets/Afareet/Scripts/Race/AiPowerUpUsagePolicy.cs")
        public_start = source.index("public static AiPowerUpDecision Decide(")
        internal_start = source.index("internal static AiPowerUpDecision Decide(", public_start)
        public_method = source[public_start:internal_start]

        self.assertIn("if (snapshot == null)", public_method)
        self.assertIn("if (inventory == null)", public_method)
