...
class HeroBlenderProductionPreflightTests(unittest.TestCase):
    ...
    def test_lod_classifier_requires_exactly_one_explicit_marker(self):
        self.assertEqual(MODULE.classify_lod("AK_Body_LOD0"), 0)
        self.assertEqual(MODULE.classify_lod("AK_Wheel_FL_LOD1"), 1)
        self.assertEqual(MODULE.classify_lod("AK_Diffuser_LOD2"), 2)
        self.assertIsNone(MODULE.classify_lod("AK_Body"))
        self.assertIsNone(MODULE.classify_lod("AK_Body_LOD0_LOD1"))
    ...
    def test_uv_and_applied_scale_are_required_for_every_lod(self):
        policy = MODULE.parse_policy(POLICY)
        totals = self.valid_totals()
        totals[1]["uv0MissingObjects"] = ["AK_Body_LOD1"]
        totals[2]["unappliedScaleObjects"] = ["AK_Wheel_FL_LOD2"]
        result = MODULE.evaluate_lod_totals(totals, policy)
        self.assertFalse(result["technicalPreflightPassed"])
        self.assertTrue(any("missing UV0" in error for error in result["errors"]))
        self.assertTrue(any("unapplied scale" in error for error in result["errors"]))

    def test_missing_lod_fails_closed(self):
        policy = MODULE.parse_policy(POLICY)
        totals = self.valid_totals()
        del totals[2]
        result = MODULE.evaluate_lod_totals(totals, policy)
        self.assertFalse(result["technicalPreflightPassed"])
        self.assertIn("LOD2 has no mesh objects", result["errors"])
