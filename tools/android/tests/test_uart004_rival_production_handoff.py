...
class RivalProductionHandoffTests(unittest.TestCase):
    ...
    @staticmethod
    def write_mtl(root: Path, name: str, *, texture_mapped: bool = True, texture_ref="rival_albedo.png") -> Path:
        ...
        for lod in range(3):
            ...
            if texture_mapped:
                lines.append(f"map_Kd {texture_ref}")
        path = root / name
    ...
    def test_exact_three_source_handoff_passes_without_acceptance_promotion(self):
        ...
        for key in (
            "productionGate", "visualAcceptance", "ownerApproval", "provenanceAccepted",
            "licensedUnityImportVerified", "physicalDeviceVerified", "verified",
        ):
    ...
    def test_three_variants_must_not_reuse_identical_obj_bytes(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            paths = [
                self.write_obj(root, variant, unique_comment=False, shared_mtl_name="shared.mtl")
    ...
    def test_every_used_material_must_resolve_to_existing_texture_map(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
    ...
    def test_texture_cannot_escape_handoff_package_even_if_target_exists(self):
        with tempfile.TemporaryDirectory() as directory:
            outer = Path(directory)
            package = outer / "package"
            package.mkdir()
            (outer / "outside.png").write_bytes(b"outside")
            self.write_mtl(package, "rival_1.mtl", texture_ref="../outside.png")
            path = self.write_obj(package, 0, mtl_reference="rival_1.mtl")
            parsed = MODULE.parse_obj(path, self.policy, 0)
            with self.assertRaisesRegex(MODULE.HandoffError, "escapes the handoff package root"):
                MODULE.validate_mtl_and_textures(path, parsed)
