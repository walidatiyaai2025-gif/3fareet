...
This tool validates technical handoff properties only. Passing it never means the source is
...
def evaluate_lod_totals(
    ...
    for lod in EXPECTED_LODS:
        ...
        if not vertex_ok:
            errors.append(
                f"LOD{lod} vertex count {vertices} is outside [{minimum_vertices}, {vertex_budget}]"
            )
        if not triangle_ok:
            errors.append(
                f"LOD{lod} triangle count {triangles} is outside [{minimum_triangles}, {triangle_budget}]"
            )
        if uv_missing:
...
def build_report(evaluation: dict[str, Any], *, source_file: str, wheel_check: dict[str, Any]) -> dict[str, Any]:
    errors = list(evaluation.get("errors", []))
    if not wheel_check.get("passed", False):
        errors.extend(wheel_check.get("errors", []))

    passed = bool(evaluation.get("technicalPreflightPassed")) and bool(wheel_check.get("passed"))
    return {
        "schemaVersion": 1,
        "task": EXPECTED_TASK,
...
def collect_blender_scene() -> tuple[dict[int, dict[str, Any]], dict[str, Any], str]:
    try:
    ...
    return lod_totals, wheel_check, str(Path(bpy.data.filepath).resolve()) if bpy.data.filepath else "<unsaved>"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        lod_totals, wheel_check, source_file = collect_blender_scene()
        evaluation = evaluate_lod_totals(lod_totals, policy)
        report = build_report(evaluation, source_file=source_file, wheel_check=wheel_check)
        if args.output:
