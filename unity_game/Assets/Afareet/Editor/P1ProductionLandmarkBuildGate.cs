...
{
    /// <summary>
    /// Fail-closed UART-006 Android gate. A candidate cannot package the owner-rejected
    /// procedural landmark path as production art. Exact tracked sources must be promoted,
    /// staged and importable before Android build execution is allowed.
    /// </summary>
    public sealed class P1ProductionLandmarkBuildGate : IPreprocessBuildWithReport
    {
        private const string ManifestRelativePath = "docs/assets/02_tracks_environments/cairo_landmarks/ASSET_MANIFEST.json";
        ...
        public static void ValidateAndroidCandidateOrThrow()
        {
            ...
            if (!string.Equals(manifest.taskId, "UART-006", StringComparison.Ordinal))
                Fail($"Unexpected Cairo landmark task id: {manifest.taskId ?? "<null>"}");
            if (!string.Equals(manifest.reviewState, ProductionReadyState, StringComparison.Ordinal))
                Fail($"UART-006 is {manifest.reviewState ?? "<null>"}; expected {ProductionReadyState} before an Android candidate can be built.");
            ...
            if (!manifest.runtimeIntegrated || !manifest.runtimeIntegrationVerified)
                Fail("UART-006 runtime integration is not verified.");
            if (manifest.proceduralFallbackAllowedInCandidate)
                Fail("UART-006 manifest allows procedural landmark fallback in a candidate build.");
            if (manifest.modules == null || manifest.modules.Length < 4)
                Fail("UART-006 production manifest must define pyramid, minaret, dome and bridge modules.");
            ...
            P1LandmarkAssetStager.StageTrackedSourcesOrThrow();
            ...
            foreach (var module in manifest.modules)
            {
                ...
                if (filters == null || filters.Length == 0)
                    Fail($"UART-006 imported resource has no mesh filters: {resourcePath}");
            }
        ...
        private static void ValidateSource(string repositoryRoot, string sourceRoot, Module module)
        {
            ...
            if (module.productionMinVertices <= 0 || module.productionMinTriangles <= 0)
                Fail($"UART-006 {module.model} is missing positive anti-blockout geometry floors.");
        ...
        private static ObjStats ReadObjStats(string path)
        {
            ...
            foreach (var raw in File.ReadLines(path))
            {
                ...
                if (polygonVertexCount >= 3) triangles += polygonVertexCount - 2;
            }
