...
APK_PATH="${1:-unity_game/Builds/Android/afareet-unity3d-debug.apk}"
EVIDENCE_DIR="${2:-artifacts/android}"
EXPECTED_PACKAGE="com.fiftysolutions.afareetunity3d"
EXPECTED_ABI="arm64-v8a"

if [[ ! -f "$APK_PATH" ]]; then
  echo "::error::Android APK is missing: $APK_PATH"
  exit 1
fi
if [[ ! -s "$APK_PATH" ]]; then
  echo "::error::Android APK is empty: $APK_PATH"
  exit 1
fi
...
printf '%s  %s\n' "$SHA256" "$(basename "$APK_PATH")" | tee "$EVIDENCE_DIR/afareet-unity3d-debug.apk.sha256"
